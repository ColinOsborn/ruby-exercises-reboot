...
class HobbitTest < Minitest::Test
  ...
  def test_hobbit_has_the_ring_if_its_name_is_frodo
    ...
    thobbit = Hobbit.new('Sam')
    assert_equal 'Frodo', hobbit.name
    assert_equal 'Sam', thobbit.name
    hobbit.has_ring?
    assert hobbit.has_ring?
    thobbit.has_ring?
    refute thobbit.has_ring?
  ...
  def test_hobbits_are_short
    hobbit = Hobbit.new('Tiny')
    assert_equal 'Tiny', hobbit.name
    hobbit.is_short?
